...
describe('ListTableBody unit test', () => {
  ...
  const content = [
    { 'id': 1, 'name': 'mock name', 'trueField': true, 'falseField': false },
    { 'id': 2, 'name': 'mock name 2', 'trueField': true, 'falseField': false }
  ];
  const contentColumnList = ['id', 'name', 'trueField', 'falseField'];
...
describe('ListTableBody functional test', () => {
  it('renders table body correctly, including boolean fields (True/False icons', () => {
    ...
    const contentColumnList = ['id', 'name', 'trueField', 'falseField'];
    const wrapper = mountWrap(<table><ListTableBody content={content} contentColumnList={contentColumnList} buttons={buttons}/></table>);
