...
export default function createDeleteRowButton(button, recordId) {
  const { rowsPerPage, page, rowTotal, sortColumn } = button.parameter;
  return (
    <Mutation
      key={`delete_${recordId}`}
      mutation={button.parameter.repository.delete()}
      variables={{ 'id': recordId }}
      refetchQueries={[{ 'query': button.parameter.repository.getListPage(), 'variables': { 'first': rowsPerPage, 'offset': rowsPerPage * page, 'orderBy': sortColumn } }]}
      onCompleted={() => {
