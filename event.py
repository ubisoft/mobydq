...
def log_event(indicator_id, batch_id, event, data_set=None):
    ...
    * Returns the corresponding event object
    ...
    indicator_list = DbOperation('Indicator').read(id=indicator_id)
    ...
    batch_list = DbOperation('Batch').read(id=batch_id, statusId=1)
    ...
    if event == 'Start':
        ...
        session_list = DbOperation('Session').read(indicatorId=indicator_id, batchId=batch_id, statusId=1)
        ...
        session = DbOperation('Session').create(indicatorId=indicator_id, batchId=batch_id, statusId=1)

        # Insert start event
        event = DbOperation('Event').create(eventTypeId=event_type_list[0].id, sessionId=session.id, content=data_set)
    ...
    # Log stop event, update running session to succeeded
        ...
        # Insert stop event
        event = DbOperation('Event').create(eventTypeId=event_type_list[0].id, sessionId=session_list[0].id, content=data_set)

        # Update running session to succeeded
        DbOperation('Session').update(id=session_list[0].id, statusId=2)
    ...
    # Log error, update running session to failed
        ...
        DbOperation('Session').update(id=session_list[0].id, statusId=3)
