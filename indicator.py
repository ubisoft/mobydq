...
def execute(indicator_id, batch_id):
    ...
    indicator_parameter_list = DbOperation('IndicatorParameter').read(indicatorId=indicator_id)
    ...
    indicator_type_list = DbOperation('IndicatorType').read(id=indicator_list[0].indicatorTypeId)
...
def get_data_frame(data_source_name, request):
    ...
    # Get data source
    data_source_list = DbOperation('DataSource').read(name=data_source_name)
    ...
    data_source_type_list = DbOperation('DataSourceType').read(id=data_source.dataSourceTypeId)
...
def compute_indicator_result(indicator_id, session_id, parameters, result_data_frame):
    ...
    DbOperation('IndicatorResult').create(
...
if __name__ == '__main__':
    ...
    indicator_list = DbOperation('Indicator').read(id=arguments.Id)
