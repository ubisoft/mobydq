...
class DataSourceType:
	...
	def create(self):
		...
		with Database() as connection:
			data = connection.executeQuery(insertRecord)
	...
	def delete(self):
		...
		selectRecord = "select data_source from data_source where data_source_type_id='{}';".format(data[0]['data_source_type_id'])
		with Database() as connection:
			dataSource = connection.executeQuery(selectRecord)
			if len(dataSource) != 0:
				log.info('Data source type {} is used by the following data sources and cannot be deleted: {}'.format(self.name, dataSource))
				return(False)

		# Confirm and execute query.
		if input('This will delete the data source type. Are you sure (Y/n): ') == 'Y':
			deleteDataSourceType = "delete from data_source_type where data_source_type_id='{}';".format(data[0]['data_source_type_id'])
			with Database() as connection:
...
class DataSource:
	...
	def get(self):
		...
		if not self.name:
			selectRecord = "select b.data_source_type, a.* from data_source a inner join data_source_type b on a.data_source_type_id= b.data_source_type_id order by data_source_id;"
		else:
			selectRecord = "select b.data_source_type, a.* from data_source a inner join data_source_type b on a.data_source_type_id= b.data_source_type_id where data_source='{}';".format(self.name)
	...
	def update(self, newName=None):
		...
		# Execute query.
		updateRecord = """update data_source set data_source='{}', data_source_type_id='{}', connection_string='{}', login='{}', password='{}', last_updated_date=strftime('%Y-%m-%d %H:%M:%f', 'now')
		where data_source_id='{}';""".format(newName, dataSourceTypeId, self.connectionString, self.login, self.password, dataSourceId)
		with Database() as connection:
			connection.executeQuery(updateRecord)
	...
	def delete(self):
		...
		with Database() as connection:
			...
			if len(indicator) != 0:
				log.info('Data source {} is used by the following indicators and cannot be deleted: {}'.format(self.name, indicator))
				return(False)
