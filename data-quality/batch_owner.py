...
class BatchOwner:
    ...
    def get(self):
        """
        Return a batch owner or a list of batch owners.
        If no batch owner name is provided return all batch owners.
        """
        log.info('Get batch owners')
        if not self.name:
            selectRecord = "select * from batch_owner order by batch_owner_id;"
        else:
            selectRecord = "select * from batch_owner where batch_owner='{}';".format(self.name)
        connection = Database()
    ...
    def create(self):
        ...
        if not self.name:
            self.name = input('Enter batch owner name: ')
    ...
    def update(self, newName=None):
        ...
        if not newName:
            newName = input('Enter new batch owner name: ')
        updateRecord = """update batch_owner set batch_owner='{}', last_updated_date=strftime('%Y-%m-%d %H:%M:%f', 'now')
            where batch_owner='{}';".format(newName, self.name)"""
        connection = Database()
